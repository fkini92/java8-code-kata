...
public class Exercise8Test extends ClassicOnlineStore {
    ...
    public void itemsNotOnSale() {
        ...
        Set<String> itemSetNotOnSale = customerStream.flatMap(customer -> customer.getWantToBuy().stream())
                .map(Item::getName)
                .filter(item -> itemListOnSale.stream().noneMatch(onSale -> onSale.equals(item)))
    ...
    public void havingEnoughMoney() {
        ...
        Predicate<Customer> havingEnoughMoney = customer -> customer.getWantToBuy().stream()
                .mapToInt(itemWanted -> onSale.stream().filter(itemOnSale -> itemOnSale.getName().equals(itemWanted.getName()))
                        .map(Item::getPrice)
                        .min(Comparator.naturalOrder()).orElse(0)).sum() <= customer.getBudget();
